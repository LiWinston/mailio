...
namespace mailio {
    ...
    const string mime::MESSAGE_ID_REGEX = "([a-zA-Z0-9\\!#\\$%&'\\*\\+\\-\\./=\\?\\^\\_`\\{\\|\\}\\~]+)"
            "\\@([a-zA-Z0-9\\!#\\$%&'\\*\\+\\-\\./=\\?\\^\\_`\\{\\|\\}\\~]+)";
    const string mime::MESSAGE_ID_REGEX_NS =
            "([a-zA-Z0-9\\!#\\$%&'\\*\\+\\-\\./=\\?\\^\\_`\\{\\|\\}\\~\\@\\\\ \\t<\\>]*)";
    ...
    mime::mime() : version_("1.0"), line_policy_(codec::line_len_policy_t::RECOMMENDED),
                   decoder_line_policy_(codec::line_len_policy_t::RECOMMENDED), strict_mode_(false),
                   strict_codec_mode_(false),
                   header_codec_(header_codec_t::UTF8), content_type_(media_type_t::NONE, ""),
                   encoding_(content_transfer_encoding_t::NONE),
                   disposition_(content_disposition_t::NONE), parsing_header_(true),
...
void mime::content(const u8string& content_str)
{
    ...
    string mime::format_content_type() const {
        ...
        if (content_type_.type != media_type_t::NONE) {
            ...
            if (!name_.buffer.empty()) {
                ...
                const string::size_type line_new_size =
                        line.size() + ATTRIBUTES_SEPARATOR_STR.size() + ATTRIBUTE_NAME.size() + NAME_VALUE_SEPARATOR_STR
                        .size() +
    ...
    string mime::format_transfer_encoding() const {
        ...
        switch (encoding_) {
            ...
            case content_transfer_encoding_t::QUOTED_PRINTABLE:
                line += CONTENT_TRANSFER_ENCODING_HEADER + HEADER_SEPARATOR_STR +
                        CONTENT_TRANSFER_ENCODING_QUOTED_PRINTABLE + codec::END_OF_LINE;
                break;
    ...
    string mime::format_content_disposition() const {
        ...
        switch (disposition_) {
            case content_disposition_t::ATTACHMENT: {
                ...
                const string::size_type line_new_size =
                        CONTENT_DISPOSITION_HEADER.size() + HEADER_SEPARATOR_STR.size() + CONTENT_DISPOSITION_ATTACHMENT
                        .size() +
                        ATTRIBUTE_FILENAME.size() + sizeof(codec::EQUAL_CHAR) + sizeof(codec::QUOTE_CHAR) + name.size()
                        + sizeof(codec::QUOTE_CHAR) +
            ...
            case content_disposition_t::INLINE: {
                string name = format_mime_name(name_);
                line += CONTENT_DISPOSITION_HEADER + HEADER_SEPARATOR_STR + CONTENT_DISPOSITION_INLINE +
                        ATTRIBUTES_SEPARATOR_STR;
                const string::size_type line_new_size =
                        CONTENT_DISPOSITION_HEADER.size() + HEADER_SEPARATOR_STR.size() + CONTENT_DISPOSITION_INLINE.
                        size() +
                        ATTRIBUTE_FILENAME.size() + NAME_VALUE_SEPARATOR_STR.size() + sizeof(codec::QUOTE_CHAR) + name.
                        size() + sizeof(codec::QUOTE_CHAR) + codec::END_OF_LINE.size();
                if (line_new_size >= string::size_type(line_policy_) - codec::END_OF_LINE.length())
                    line += codec::END_OF_LINE + NEW_LINE_INDENT;
    ...
    string mime::format_content_id() const {
        return content_id_.empty()
                   ? ""
                   : CONTENT_ID_HEADER + HEADER_SEPARATOR_STR + format_many_ids(content_id_) + codec::END_OF_LINE;
    }
    ...
    */
    ...
    void mime::parse_content_transfer_encoding(const string &transfer_encoding_hdr,
    ...
    */
    void mime::parse_header_value_attributes(const string &header, string &header_value,
                                             attributes_t &attributes) const {
        enum class state_t {
            BEGIN, VALUE, ATTR_BEGIN, ATTR_NAME, PRE_EQUAL, ATTR_SEP, POST_EQUAL, QATTR_VALUE_BEGIN, ATTR_VALUE_BEGIN,
            ATTR_VALUE_END, END
        };
    ...
    std::string urlDecode(const std::string &str) {
        ...
        for (size_t i = 0; i < str.length(); ++i) {
            if (str[i] == '%' && i + 2 < str.length() && isxdigit(str[i + 1]) && isxdigit(str[i + 2])) {
                // 解码 URL 编码的字符
                char decodedChar = static_cast<char>(std::stoi(str.substr(i + 1, 2), nullptr, 16));
                decoded << decodedChar;
                i += 2; // 跳过两个十六进制字符
            } else if (str[i] == '+') {
                // 将 '+' 替换为空格
    ...
    string_t mime::parse_header_value_attribute(const string &attr_value) const {
        ...
        auto attr_view_dec = urlDecode(attr_value.substr(language_pos + 1));
        stringstream dec_ss;
